...
public class TarefaDAO extends Tarefa {
	...
	public void cadastrar() {
		...
		bd = new BD();
		String sql = 
				"INSERT INTO tarefa(id_centro_custo,id_tamanho,id_departamento,processo_relacionado,descri,prioridade,stat,porcentagem,prazo,predecessor_1,predecessor_2,predecessor_3,checado,data_ini,data_real,data_fim,pendente_por,status_pendencia,historico,responsavel,autoridade,dpto_correto,id_pessoa,id_update,etapa,subetapa)"
				+ "SELECT"
				...
				+ " ? AS id_pessoa,"
		        + " ? AS id_update,"
		        + "(SELECT etapas.id_etapa FROM etapas WHERE etapas.etapa = ?),"
		        + "(SELECT sub_etapas.id_sub_etapas FROM sub_etapas WHERE sub_etapas.sub_etapa = ?)";
		...
		if(bd.getConnection()) {//se conectar com o banco continua...
			...
			try {
				...
				bd.st.setString(25, getEtapa());
				bd.st.setString(26, getSubEtapa());
	...
	public void atualizar() {
		...
		String sql = "UPDATE tarefa \n\r"
                ...
                + "tarefa.etapa = (SELECT id_etapa FROM etapas WHERE etapa = ?),"
                + "tarefa.subetapa = (SELECT id_sub_etapas FROM sub_etapas WHERE sub_etapa = ?),"
                + "tarefa.id_update=?,"
                + "executor.executor1=?,"
                + "executor.porcento1=?,"
                + "executor.executor2=?,"
		...
		if(bd.getConnection()){
			try {
				...
				bd.st.setString(23, getEtapa());
				bd.st.setString(24, getSubEtapa());
